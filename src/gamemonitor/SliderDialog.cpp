...
{
	/* create frame */
	Create(parent,-1, L"Select time", pos, size,
	...
	record_size =
		...
		+ sizeof(ConnectedServerInfo) * map_data.num_servers /* servers */
		+ 3 * sizeof(int) /* quest */
	...
	loc.Create(this, -1, L"", wxPoint(x-100,y-20), wxSize(90,20), wxALIGN_RIGHT);
...
void SliderDialog::loadPosition(int pos)
{
	...
	wxString label;
	label.Printf(L"%d/%d", pos,records);
	loc.SetLabel(label);

	sr->seek(sizeof(MapData) + pos * record_size);

	sr->read(&timestamp, sizeof(Uint32));
	sr->read(&ss[0], sizeof(ConnectedServerInfo) * map_data.num_servers);
