...
MessageWithSerializator::MessageWithSerializator(int t) : Message(t)
{
	/* alocate space for a new packet */
	packet = SDLNet_AllocPacket((int)MAX_UPD_PACKET_SIZE);
	if ( packet == NULL ) return;
	memset(packet->data, 0, MAX_UPD_PACKET_SIZE);
	packet->len = MAX_UPD_PACKET_SIZE;
	...
	#endif
	if ( s != NULL ) *s << (int)message_type;
}

MessageWithSerializator::MessageWithSerializator(UDPpacket *p) : Message(0)
{
	if ( p == NULL ) return;
