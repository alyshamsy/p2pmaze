...
public:
	...
	struct ChunkHeader {
		...
		Uint32  len;	// The lenght of the chunk
	};

	// I sort the mesh by material so that I won't have to switch textures a great deal
	struct MaterialFaces {
		Uint16 *subFaces;	// Index to our vertex array of all the faces that use this material
	...
	struct Object {
		...
		Uint16 *Faces;		// The array of face indices
...
private:
	void IntColorChunkProcessor(Sint32 length, Sint32 findex, int matindex);
	void FloatColorChunkProcessor(Sint32 length, Sint32 findex, int matindex);
	// Processes the Main Chunk that all the other chunks exist is
	void MainChunkProcessor(Sint32 length, Sint32 findex);
		// Processes the model's info
		void EditChunkProcessor(Sint32 length, Sint32 findex);

			// Processes the model's materials
			void MaterialChunkProcessor(Sint32 length, Sint32 findex, int matindex);
				// Processes the names of the materials
				void MaterialNameChunkProcessor(Sint32 length, Sint32 findex, int matindex);
				// Processes the material's diffuse color
				void DiffuseColorChunkProcessor(Sint32 length, Sint32 findex, int matindex);
				// Processes the material's texture maps
				void TextureMapChunkProcessor(Sint32 length, Sint32 findex, int matindex);
					// Processes the names of the textures and load the textures
					void MapNameChunkProcessor(Sint32 length, Sint32 findex, int matindex);

			// Processes the model's geometry
			void ObjectChunkProcessor(Sint32 length, Sint32 findex, int objindex);
				// Processes the triangles of the model
				void TriangularMeshChunkProcessor(Sint32 length, Sint32 findex, int objindex);
					// Processes the vertices of the model and loads them
					void VertexListChunkProcessor(Sint32 length, Sint32 findex, int objindex);
					// Processes the texture cordiantes of the vertices and loads them
					void TexCoordsChunkProcessor(Sint32 length, Sint32 findex, int objindex);
					// Processes the faces of the model and loads the faces
					void FacesDescriptionChunkProcessor(Sint32 length, Sint32 findex, int objindex);
						// Processes the materials of the faces and splits them up by material
						void FacesMaterialsListChunkProcessor(Sint32 length, Sint32 findex, int objindex, int subfacesindex);
